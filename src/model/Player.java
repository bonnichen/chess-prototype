...
public class Player {
    ...
    private boolean canSelect(int row, int col) {
        ...
        return selected != null && selected.getPlayer() == model.getPlayerInPlay() && selected.isCaptured() == false;
    }
    ...
    public void moveSimulation(GraphCoord move, Pieces selectedPiece) {
        selectedPiece.setPos(move.getCoord().i(), move.getCoord().j());
        validMoves = null;
        model.setLastMove(selectedPiece.getLastMove());

        Pieces pieceMoved = selectedPiece;
        if (pieceMoved instanceof Pawn && ((Pawn) pieceMoved).canPromote()) {
            ((Pawn) pieceMoved).processPromotion(1);
        }
    }
